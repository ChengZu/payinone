...
public class WXPayDomainSimpleImpl implements IWXPayDomain {
	...
	public synchronized void report(final String domain,
			long elapsedTimeMillis, final Exception ex) {
	...
	public synchronized DomainInfo getDomain(final WXPayConfig config) {
		...
		if (switchToAlternateDomainTime == 0) { // first switch
			...
			DomainStatics alternateDomain = domainData
					.get(WXPayConstants.DOMAIN_API2);
	...
	static class DomainStatics {
		...
		int badCount() {
			return connectTimeoutCount + dnsErrorCount * 5 + otherErrorCount
					/ 4;
		}
	}
