...
layui.config({
	...
	var layer = parent.layer === undefined ? layui.layer : parent.layer;
		...
		$("#username").val(window.sessionStorage.getItem('username'));
        loadProvince();
        layui.upload({
        ...
        form.verify({
            oldPwd : function(value, item){
            	//var password = window.sessionStorage.getItem("password").toLowerCase();
        ...
        form.on("submit(changePwd)",function(data){
        	var password=md5($("#confirmPwd").val()).toUpperCase();
        	var index = layer.msg('提交中，请稍候',{icon: 16,time:false,shade:0.8});
